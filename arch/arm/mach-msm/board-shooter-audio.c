...
void shooter_snddev_poweramp_on(int en)
{
	...
	if (en) {
		...
		gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_SPK_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_SPK_ENO), 1);
		...
		gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_SPK_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_SPK_ENO), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
...
void shooter_snddev_hsed_pamp_on(int en)
{
	...
	if (en) {
		/* enable rx route */
		msleep(30);
		gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_HANDSET_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_HANDSET_ENO), 1);
		set_headset_amp(1);
		...
		gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_HANDSET_ENO),
						"AUD_HP_EN");
		gpio_direction_output(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_HANDSET_ENO), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_HEADSET;
	}
}
...
void shooter_snddev_imic_pamp_on(int en)
{
	...
	if (en) {
		...
		gpio_request(PM8058_GPIO_PM_TO_SYS(SHOOTER_AUD_MIC_SEL),
						"AUD_MIC_SEL");
