...
static bool mhl_detect_internal = false;
#endif
...
static int cable_detect_get_adc(void)
{
	...
	ret = pm8058_htc_config_mpp_and_adc_read(
				...
				pInfo->mpp_data->usbid_mpp,
				pInfo->mpp_data->usbid_amux);
	if (ret)
		CABLE_ERR("Get usb id adc data failed.");
	for (i = 0; i < 5; i++)
		mean_adc += adc[i];

	mean_adc /= 5;
	CABLE_INFO("%s: MPP ADC %d\n", __func__, mean_adc);

	result = PM8058ADC_15BIT(mean_adc);
	CABLE_INFO("%s: result = %d\n", __func__, result);
	return result;
}
	...
	if (!isMHL) {
		...
		if (vbus && (charging_type > CONNECT_TYPE_NONE)) {
			...
			if (charging_type == CONNECT_TYPE_INTERNAL)
				charging_type =
					mhl_detect_internal?CONNECT_TYPE_USB:CONNECT_TYPE_INTERNAL;
